...
export function CharacterDetailPanel({ cardId, onEdit, onClose }: CharacterDetailPanelProps) {
  ...
  // Large cards can be huge; avoid re-parsing/re-building derived objects on unrelated rerenders.
  // Important: keep hooks unconditional (no early returns before this).
  const dashData = useMemo<CharacterDetailDashboardData | null>(() => {
    if (!card) return null;
    ...
    const specLabel = card.spec ? card.spec.replace('chara_card_', '').toUpperCase() : 'V1';
